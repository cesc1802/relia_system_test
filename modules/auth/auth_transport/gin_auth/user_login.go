...
func UserLogin(appCtx app_context.AppCtx) gin.HandlerFunc {
	return func(c *gin.Context) {
		var data auth_dto.LoginRequest
		...
		repo := auth_repository.NewLoginRepository(store, tokProvider, md5)

		result, err := repo.UserLogin(c.Request.Context(), &data)
